...
function Fight({ enemyHp, setEnemyHp, enemyAttack, enemyDefense, userHp, setUserHp, userAttack, userDefense, userPokemonName }) {
  ...
  return (
    <>
      {hasFightStarted ? null : (
        <button onClick={startFight} disabled={isFightOver}>
          Let's GO!
        </button>
      )}
      {isFightOver && (
